// 🚗 Componente de autocompletado dinámico para Vehículos Usados
...
import {
  ...
  useModelosByMarca,
  getOptionsFromData,
} from "@/hooks/useUsadosOptions";
import { Loader2 } from "lucide-react";

interface DynamicUsadosAutocompleteProps {
  id: string;
  name: string;
  value: string;
...
interface DynamicUsadosModeloAutocompleteProps {
  id: string;
  name: string;
  value: string;
  onChange: (value: string) => void;
...
export const DynamicUsadosAutocomplete: React.FC<
  DynamicUsadosAutocompleteProps
> = ({
  id,
  name,
  value,
  ...
  const { options, loading, error } = useUsadosOptions({
    isAdmin,
    enableFallback: true,
  });
...
export const DynamicUsadosModeloAutocomplete: React.FC<
  DynamicUsadosModeloAutocompleteProps
> = ({
  id,
  name,
  value,
